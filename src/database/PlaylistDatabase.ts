...
export class PlaylistDatabase extends BaseDatabase {
    ...
    public update = async (id: string, playlistDB: PlaylistDB): Promise<void> => {
    ...
    public findPlaylistsWithCreatorsById = async (playlistId: string): Promise<PlaylistWithCreatorDB | undefined> => {
        const result: PlaylistWithCreatorDB[] = await BaseDatabase
            .connection(PlaylistDatabase.TABLE_PLAYLISTS)
            .select(
                "playlists.id",
    ...
    public likeOrDislikePlaylist = async (likeDislike: LikeDislikeDB): Promise<void> => {
        await BaseDatabase.connection(PlaylistDatabase.TABLE_LIKES_DISLIKES)
            .insert(likeDislike)
    }

    public findLikeDislike = async (likeDislikeDBToFind: LikeDislikeDB): Promise<PLAYLIST_LIKE| null> => {
        ...
        if (likeDislikeDB) {
            return likeDislikeDB.like === 1 ? PLAYLIST_LIKE.ALREADY_LIKED: PLAYLIST_LIKE.ALREADY_DILIKED
        } else {
            return null
        }
    }
